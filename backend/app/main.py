...
from app.routes import status_pages, settings, notifications, backup, reports
...
def get_app_version() -> str:
    """Read the version baked into the image at /VERSION (see Dockerfile)."""
    for vf in ("/VERSION", os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "VERSION")):
        try:
            with open(vf, "r") as f:
                return f.read().strip()
        except OSError:
...
def _find_frontend_dist() -> str | None:
    ...
    Exactly one location, no fallback chain: the image bakes the build at
    /app/frontend_dist (see Dockerfile). FRONTEND_DIST can override it for
    local development.

    A previous version searched six candidate paths and silently fell back
    when one lacked index.html — which meant a half-copied directory served
    a stale bundle with nothing indicating why. If the frontend is missing
    now, the app says so loudly instead of guessing.
    """
    ...
    logger.warning(
        "No frontend build at %s (index.html missing). Serving API only. "
        "Set FRONTEND_DIST or rebuild the image.", path
    )
    return None
